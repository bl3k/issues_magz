...
<div class="row">
	<div class="col-xs-12">
    	<br>
		<?= form_open('/admin/dok_hukum/do_update', array('id' => 'form', 'role' => 'form', 'class' => 'form-horizontal')); ?>
			<input type="hidden" name="id" value="<?= $dok_hukum['dok_hukum_id']; ?>" />
			<input type="hidden" name="old_file" value="<?= $dok_hukum['file']; ?>" />
				...
				<div class="col-sm-4">
					<select name="kategori_dok_hukum_id" id="kategori_dok_hukum_id" class="form-control">
						...
						<?php foreach ($kategori_dok_hukum_list as $kategori_dok_hukum): ?>
							<option <?= $dok_hukum['kategori_dok_hukum_id'] == $kategori_dok_hukum->kategori_dok_hukum_id ? 'selected' : ''; ?> value="<?= $kategori_dok_hukum->kategori_dok_hukum_id; ?>"><?= $kategori_dok_hukum->judul; ?></option>
						<?php endforeach; ?>
					</select>
				</div>
			</div>
				...
				<div class="col-sm-4">
					<select name="bentuk_dok_hukum_id" id="bentuk_dok_hukum_id" class="form-control">
						...
						<?php foreach ($bentuk_dok_hukum_list as $bentuk_dok_hukum): ?>
							<option <?= $dok_hukum['bentuk_dok_hukum_id'] == $bentuk_dok_hukum->bentuk_dok_hukum_id ? 'selected' : ''; ?> value="<?= $bentuk_dok_hukum->bentuk_dok_hukum_id; ?>"><?= $bentuk_dok_hukum->judul; ?></option>
						<?php endforeach; ?>
					</select>
				</div>
			</div>
			...
			<div class="form-group">
				...
				<div class="col-sm-1">
					<input type="text" class="form-control" id="tahun" name="tahun" value="<?= $dok_hukum['tahun']; ?>">
				</div>
			</div>
			...
			<div class="form-group">
				...
				<div class="col-sm-4">
					<input type="text" class="form-control" id="nomor" name="nomor" value="<?= $dok_hukum['nomor']; ?>">
				</div>
			</div>
			...
			<div class="form-group">
				...
				<div class="col-sm-6">
					<textarea class="form-control" id="perihal" name="perihal"><?= $dok_hukum['perihal']; ?></textarea>
				</div>
			</div>
			...
			<div class="form-group">
				...
				<div class="col-sm-8">
					<label class="checkbox-inline">
						<input type="radio" id="is_publish" value="1" name="is_publish" <?= $dok_hukum['is_publish'] == 1 ? 'checked' : ''; ?>> Ya
					</label>
					<label class="checkbox-inline">
						<input type="radio" id="is_publish" value="0" name="is_publish" <?= $dok_hukum['is_publish'] == 0 ? 'checked' : ''; ?>> Tidak
					</label>						
				</div>
			</div>							                		
...
<script type="text/javascript">
	var options = {
		...
		success: function(responseText, statusText, xhr, $form) {
			$.unblockUI();
			loadPage("/admin/dok_hukum/index", "update", responseText);
		},
		resetForm: true
	};
