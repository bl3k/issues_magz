...
class Dok_hukum_model extends CI_Model {

	private $table_name = 'm_dok_hukum';
	private $primary_key = 'dok_hukum_id';
	...
	public function get_all_records($current=0, $row_count=0, $sort=array(), $search_phrase='') {
		...
		//$search_phrase = $this->db->escape($search_phrase);
		
		$where = 'perihal LIKE \'%' . $search_phrase . '%\' ';
		$where .= 'OR nomor=\'' . $search_phrase . '\' ';
		$where .= 'OR tahun=\'' . $search_phrase . '\'';
		...
		if ($sort) {
			...
			$result = $result->order_by($this->primary_key, 'desc');
		}
	...
	public function get_records_for_fp($is_publish) {
		...
		$this->db->order_by('created_at', 'desc')->limit(2);
