...
class News_model extends CI_Model {
	...
	private $table_name = 'm_news';
	private $primary_key = 'news_id';
	...
	public function get_all_records($current=0, $row_count=0, $sort=array(), $search_phrase='') {
		...
		$where = 'published_at=\'' . $search_phrase . '\' ';
		$where .= 'OR news_title LIKE \'%' . $search_phrase . '%\' ';
		$where .= 'OR created_by LIKE \'%' . $search_phrase . '%\' ';
	...
	public function get_record_by_params(array $data) {
		...
		$this->db->order_by('view_counter', 'desc')->limit(5);
